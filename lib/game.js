...
var config = require(__dirname + '/config/config'),
    ...
    Game = function (w, h, max) {
        ...
        this.spawnFoods = function () {
            var food = new Food(~~((Math.random() * this.w) / 10) * 10, ~~((Math.random() * this.h) / 10) * 10);
            this.foods.push(food);
            bc.emit('spawnFood', food);
        };
        ...
        this.start = function (broadcaster) {
            ...
            foodInterval = setInterval(function (g) {
                g.spawnFoods();
            }, 5000, this);

            for (i in this.players) {
                temp = this.players[i];
                temp.interval = setInterval(function (temp, d, players, foods) {
                    ...
                    i = foods.length;
                    while (i--) {
                        if (foods[i].x === temp.headX && foods[i].y === temp.headY) {
                            foods.splice(i, 1);
                            temp.length++;
                        }
                    }
                ...
                }, temp.speed, temp, {w : this.w, h : this.h}, this.players, this.foods);
            }
        };
    };
